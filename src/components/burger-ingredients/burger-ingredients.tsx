        ...
        <div className={styles.wrapper}>
            ...
            <div className={`${styles.wrapper_indredients} mt-10 scroll`}>
                ...
                <div className={`${styles.ingredient_list} mb-10`}>
                    {burgerList.filter(item => item.type === 'bun').map(item => <IngredientsCard key={item._id} {...item} />)}
                </div>
                <h2 className={styles.title}>Соусы</h2>
                <div className={`${styles.ingredient_list} mb-10`}>
                    {burgerList.filter(item => item.type === 'sauce').map(item => <IngredientsCard key={item._id} {...item} />)}
                </div>
                <h2 className={styles.title}>Начинки</h2>
                <div className={`${styles.ingredient_list} mb-10`}>
                    {burgerList.filter(item => item.type === 'main').map(item => <IngredientsCard key={item._id} {...item} />)}
                </div>
            </div>
        </div>
    )
